...
def generar_tonos_de_gris(num_tonos):
    """Genera una lista de tonos de gris."""
    return [((i * 255) // (num_tonos - 1),) * 3 for i in range(num_tonos)]

# Función para crear la imagen recursiva en tonos de gris
def crear_imagen_recursiva_gris(imagen, num_tonos=60, factor_medida=2, factor_escala=0.02):
    # Imagen reducida
    ...
    # Generar tonos de gris
    tonos_gris = generar_tonos_de_gris(num_tonos)
    ...
    for x in range(0, ancho, sub_ancho):
        for y in range(0, alto, sub_alto):
            ...
            promedio = color_promedio(bloque)
            promedio_gris = int((promedio[0] + promedio[1] + promedio[2]) / 3)
            tono_gris = (promedio_gris,) * 3

            # Si no hemos creado la imagen con el tono de gris, la creamos
            clave = str(tono_gris)
            if clave not in tonos:
                mica = Image.new('RGB', (sub_ancho, sub_alto), tono_gris)
                imagen_con_mica = Image.blend(imagen_reducida, mica, alpha=0.5)
                tonos[clave] = imagen_con_mica

            # Pegamos el bloque en la imagen recursiva
...
def aplicar_mosaico_gris():
    ...
    if imagen:
        try:
            ...
            imagen_mosaico = crear_imagen_recursiva_gris(imagen)
...
# Botón para aplicar el mosaico en tonos de gris
boton_mosaico_gris = tk.Button(frame_botones, text="Mosaico Recursivo a Grises", command=aplicar_mosaico_gris)
boton_mosaico_gris.pack(side=tk.LEFT, padx=10)
